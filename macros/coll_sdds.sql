{% macro collate_sdds () %}
 ...
 {% set region= env_var('DBT_REGION')   %}
    {% set var_drop = run_query('select QURY from HRDP_PUB_'+region+'_DB.CMN_PUB_SCH.Collate_execution_sdds_drop where is_executed=\'false\' order by RN ;') %}
      {% for var_landing1 in var_drop %}
        {% do run_query(var_landing1.QURY) %}
        {% do run_query('update  HRDP_PUB_'+region+'_DB.CMN_PUB_SCH.Collate_execution_sdds_drop set is_executed=\'True\'  where QURY= \''+var_landing1.QURY+ '\'' ) %}
    {% endfor %}   
-- landing starts --
    {% set query %}
...
insert into HRDP_PUB_{{ env_var('DBT_REGION') }}_DB.CMN_PUB_SCH.Collate_execution_sdds  (qury,table_name)
with tabs as (
...
(
  ...
  {% do run_query(query) %}
    {% set region= env_var('DBT_REGION')   %}
    {% set var_lan = run_query('select QURY from HRDP_PUB_'+region+'_DB.CMN_PUB_SCH.Collate_execution_sdds where is_executed=\'false\' order by table_name,QURY ;') %}
     
      {% for var_landing in var_lan %}
        {% do run_query(var_landing.QURY) %}
        {% do run_query('update  HRDP_PUB_'+region+'_DB.CMN_PUB_SCH.Collate_execution_sdds set is_executed=\'True\' where QURY= \''+var_landing.QURY+ '\'' ) %}
    {% endfor %}  
    ...
    {% set region= env_var('DBT_REGION')   %}
          ...
          insert into HRDP_PUB_{{ env_var('DBT_REGION') }}_DB.CMN_PUB_SCH.Collate_execution_sdds_drop  (qury)
  with tabs as (
