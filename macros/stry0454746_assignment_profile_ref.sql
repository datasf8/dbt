{% macro stry0454746_assignment_profile_ref() %}
    ...
    {% set qRestAll %}
        ...
        insert into HRDP_SDDS_{{ env_var('DBT_REGION') }}_DB.BTDP_DS_C1_H08_LEARNING_EU_{{ env_var('DBT_REGION') }}_PRIVATE.DIM_PARAM_ASSIGNMENT_PROFILE_REFERENTIAL values
            ...
            (	'AP_JBMUST_LSA_MANAGER_COACH_TARGET',	'LSA - Field Manager Coach',	'MUST',	'JBMUST',	'Job Must'	),
            ...
            (	'AP_JBMUST_LSA_TRAININGFORIMPACT_TARGET',	'Training for Business Impact',	'MUST',	'JBMUST',	'Job Must'	),
			...
			(	'AP_JBMUST_DEMANDPLANNINGMANAGEMENT_TARGET'	, 'Demand Planning Management - Essential' , 'MUST' , 'JBMUST' , 'Job Must' ),
			(	'AP_JBMUST_DIGITAL_SCENOGRAPHY_TARGET' , 'Digital scenography' , 'MUST' , 'JBMUST' , 'Job Must' ),
			(	'AP_ONBMUST_LSA_SOCIALPRO3_TARGET' , 'LSA social pro 3' , 'MUST' , 'ONBMUST', 'Onboarding Must' ),
			(	'AP_JBMUST_SRA_EMPATHY_LUXE_ADVANCED_TARGET' , 'SRA Empathy Luxe Essential' , 'MUST' , 'JBMUST' , 'Job Must' ),
			(	'AP_JBMUST_SOURCINGNEGOTIATION_TARGET' , 'Sourcing Negotiation - Essential' , 'MUST' , 'JBMUST' , 'Job Must' ),
			(	'AP_JBMUST_SOURCINGNEGOTIATIONADV_TARGET' , 'Sourcing Strategy And Implementation - Advanced' , 'MUST' , 'JBMUST' , 'Job Must' ),
			(	'AP_JBMUST_FINANCEINDIRECTSOURCING_TARGET' , 'Finance for Indirect Sourcing - Essential' , 'MUST' , 'JBMUST' , 'Job Must' ),
			(	'AP_JBMUST_FINANCEDIRECTSOURCING_TARGET' , 'Finance for direct Sourcing - Essential' , 'MUST' , 'JBMUST' , 'Job Must' ),
			(	'AP_BUSINESSMUST_INCLUSIVELEADERSHIP_TARGET' , 'Inclusive Leadership' , 'MUST','BUSINESSMUST' , 'Business Must' ),
			(	'AP_JBMUST_BU_CONTROLLER_TARGET' , 'BU Controller' , 'MUST' , 'JBMUST' , 'Job Must' ),
			(	'AP_LOCAL_MANDATORY_TRUSTWORTHYAI_TARGET' , 'Trustworthy AI' , 'MANDT' , 'LCMANDT' , 'Local Mandatory')
       ;
