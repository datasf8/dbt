...
# from pmg_api_call_config.encryption_decryption_operator import EncryptionOperator
from airflow.providers.google.cloud.operators.dataflow import DataflowCreatePythonJobOperator
from airflow.providers.snowflake.operators.snowflake import SnowflakeOperator
from airflow.providers.dbt.cloud.operators.dbt import DbtCloudRunJobOperator
from pmg_emp_goals_config import dag_config as config
...
def create_url_file_api_data(bucket_name, path, api_details, **context):
    ...
    push_to_xcom = api_details.get("PUSH_TO_XCOM")
    call_multiple_time = api_details.get("CALL_MULTIPLE_TIME")
    ...
    goal_plan_template_id_list = context["task_instance"].xcom_pull(key="goal_plan_template_ids")
    if goal_plan_template_id_list:
        goal_plan_template_id_list = json.loads(goal_plan_template_id_list)
    if not goal_plan_template_id_list:
        goal_plan_template_id_list = ["4"]
    for template_id in goal_plan_template_id_list:
        date_filter = ""
        order_by_str = ""
        ...
        url = url.format(url_id=template_id)
        ...
        while True:
            ...
            if file_data.get("d") and dependant_url_list:
                for user_data in file_data["d"]["results"]:
                    ...
                    for url_dict in dependant_url_list:
                        ...
                        select_cols = url_dict.get("SELECT_COLS")
            ...
            if push_to_xcom:
                goal_plan_template_ids = []
                for api_data in file_data["d"]["results"]:
                    goal_plan_template_ids.append(api_data.get(data_id))
                context["task_instance"].xcom_push(key="goal_plan_template_ids", value=json.dumps(goal_plan_template_ids))
            if is_store_data:
                logging.info("In data store block")
                if call_multiple_time:
                    file_path = os.path.join(file_path, sequence_table_name,
                                             f"{sequence_table_name}_{template_id}_{num}_{datetime.utcnow().strftime('%Y%m%d%H%M%S')}.json")
                else:
                    file_path = os.path.join(file_path, sequence_table_name,
                                                f"{sequence_table_name}_{num}_{datetime.utcnow().strftime('%Y%m%d%H%M%S')}.json")

                logging.info(f"File Path: {file_path}")
                logging.info(f"call_multiple_time: {call_multiple_time}")
...
def update_snowflake_timestamp(bucket_name, db_file_path, file_name, temp_file_name, **context):
    storage_client = storage.Client()
    bucket = storage_client.bucket(bucket_name)
...
try:
    with DAG(
        ...
        for task_detail in task_list:
            ...
            if task_type == "audit1":
            ...
            elif task_type == "cloudrun_create":
                job_name = task_detail.get("JOB_NAME")
                task_count = task_detail.get("TASK_COUNT")
                # IMAGE_NAME = f"eu.gcr.io/{config.project_name}/{job_name}"
                # IMAGE_NAME = f"eu.gcr.io/itg-hranalytics-gbl-ww-dv/hrdp-cr-apicall-parallel-ew1:latest"
                IMAGE_NAME = f"{config.location}-docker.pkg.dev/{config.project_name}/hrdp-cr-apicall-parallel-ew1-dv/cloud-run-perf-goals"
                task_obj = BashOperator(
                    task_id=task_id,
                    bash_command=f'''gcloud beta run jobs create {job_name} \
                        --image {IMAGE_NAME} \
                        --command python \
                        --args process.py \
                        --tasks {task_count} \
                        --set-env-vars BUCKET_NAME={config.BUCKET_NAME}\
                        --set-env-vars SOURCE_FILE_PATH={os.path.join(
                        config.DESTINATION_PATH, task_detail.get("URL_FILE_NAME"))}\
                        --set-env-vars SECRET_NAME={config.SECRET_NAME}\
                ...
                task_obj = BashOperator(
                    ...
                    bash_command=f'''gcloud beta run jobs execute {job_name} \
                                    ...
                                    --args process.py \
                                    --update-env-vars SOURCE_FILE_PATH={config.DESTINATION_PATH}/employee_goals_list \
                                    --tasks {task_count} \
                                    --wait''',
                    dag=dag,
                )
            elif task_type == "cloudrun_list":
                task_obj = CloudRunListJobsOperator(
                    task_id=task_id, project_id=config.project_name, region=config.zone, dag=dag
                )
