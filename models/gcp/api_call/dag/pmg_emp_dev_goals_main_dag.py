...
from pmg_emp_dev_goals_config import dag_config as config
from pipeline.pipeline_config import dag_config as pipe_config
...
def create_url_file_api_data(bucket_name, path, api_details, **context):
    ...
    dev_goal_plan_template_id_list = context["task_instance"].xcom_pull(key="dev_goal_plan_template_ids")
    if dev_goal_plan_template_id_list:
        dev_goal_plan_template_id_list = json.loads(dev_goal_plan_template_id_list)
    if not dev_goal_plan_template_id_list:
        dev_goal_plan_template_id_list = ["2003"]
    for template_id in dev_goal_plan_template_id_list:
        date_filter = ""
        order_by_str = ""
                    ...
                    if sub_list_str:
                        for url_dict in dependant_url_list:
                            ...
                            if is_combined_filter:
                                ...
                                url_data += user_url
            if push_to_xcom:
                dev_goal_plan_template_ids = []
                for api_data in file_data["d"]["results"]:
                    dev_goal_plan_template_ids.append(api_data.get(data_id))
                context["task_instance"].xcom_push(key="dev_goal_plan_template_ids", value=json.dumps(dev_goal_plan_template_ids))
