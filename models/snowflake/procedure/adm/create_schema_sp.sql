{{
    config(
        ...
        pre_hook ="
CREATE OR REPLACE PROCEDURE HRDP_ADM_{{ env_var('DBT_REGION') }}_DB.DB_SCHEMA_SCH.CREATE_SCHEMA_SP(DATABASE_NAME VARCHAR(16777216), SCHEMA_NAME VARCHAR(16777216))
...
AS '
    ...
    snowflake.createStatement( {sqlText: `use role `+v_own_rl+`;`} ).execute();
