...
with
            ...
            (
                select
                    ...
                    case
                        when
                            dt_begin >= coalesce(
                                employee_group.employee_group_start_date, '1900-01-01'
                            )
                        then dt_begin
                    ...
                    case
                        when
                            dt_end <= coalesce(
                                employee_group.employee_group_end_date, '9999-12-31'
                            )
                        then dt_end
                        else
                            coalesce(
                ...
                from {{ ref("_tmp_csrd_goegraphic_zone") }} geographic_zone

                left outer join
                    job_information_v1 job_info_init
                    on geographic_zone.user_id = job_info_init.user_id
                    and geographic_zone.dt_begin <= job_info_init.job_end_date
                    and geographic_zone.dt_end >= job_info_init.job_start_date

                join
                    {{ ref("employee_group_v1") }} employee_group
                    on job_info_init.employee_group_code
                    = employee_group.employee_group_code
                    and job_info_init.job_start_date
                    <= employee_group.employee_group_end_date
                    and job_info_init.job_end_date
                    >= employee_group.employee_group_start_date

            )
    ),
    employee_group_intervals as (
        ...
        from
            (
                ...
                from employee_group_cte beg_inter
                inner join
                    employee_group_cte end_inter
                    on (
    ...
    employee_group_indicator as (
        ...
        from employee_group_intervals
    )
select *
from employee_group_indicator