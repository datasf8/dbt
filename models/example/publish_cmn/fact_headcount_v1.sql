{{
    config(
        ...
        call rls_policy_apply_sp(
        '{{ database }}','{{ schema }}','FACT_HEADCOUNT_V1');"
    )
}}
with
    mnth_end as (
    ...
    headcount as (
        select hc.*, me.headcount_date
        from {{ ref("headcount_v1") }} hc
        join
            mnth_end me
            on me.headcount_date between hc.job_start_date and hc.job_end_date
        where hc.headcount_present_flag = 1
    ),
    ji_pos as (
        select *
        from job_info
    ...
    mgr as (
        ...
        join
            job_info j1
            on hc.user_id = j1.user_id
            and hc.job_start_date = j1.job_start_date
        left join
            {{ ref("position_v1") }} p1
            on j1.position_code = p1.position_code
            and hc.headcount_date
            between p1.position_start_date and p1.position_end_date
        left join
            ji_pos j2
            on p1.higher_position_code = j2.position_code
            and hc.headcount_date between j2.job_start_date and j2.job_end_date
        qualify
            row_number() over (
                partition by hc.user_id, hc.headcount_date
    ...
    final as (
        select
            ...
            bi.date_of_birth,
            round(
                months_between(hc.headcount_date, ji.job_entry_date), 3
            ) job_seniority_months,
            round(
                months_between(hc.headcount_date, group_seniority_date), 3
            ) group_seniority_months,
            round(
                months_between(hc.headcount_date, bi.date_of_birth), 3
            ) age_seniority_months
        from headcount hc
        join
            job_info_calc ji
            on hc.user_id = ji.user_id
        ...
        left join
            mgr on hc.user_id = mgr.user_id and hc.headcount_date = mgr.headcount_date
        left join
            hr on hc.user_id = hr.user_id and hc.headcount_date = hr.headcount_date
    )  -- select * from final
select
    nvl(emp.employee_sk, -1) as employee_sk,
    f.user_id,
    f.personal_id,
    to_char(f.headcount_date, 'YYYYMMDD')::integer as headcount_date_sk,
    to_char(f.headcount_date, 'YYYYMM')::integer as headcount_month_sk,
...
join
    ...
    and f.headcount_date between organization_start_date and organization_end_date
    and org.is_ec_live_flag = true
left join
    {{ ref("dim_employee_v1") }} emp
    on f.user_id = emp.user_id
    and f.headcount_date between emp.employee_start_date and emp.employee_end_date
left join
    {{ ref("dim_job") }} job
...
left join
    ...
    and f.headcount_date between position_start_date and position_end_date
left join
...
left join
    ...
    and f.headcount_date between mgr.employee_start_date and mgr.employee_end_date
left join
    {{ ref("dim_employee_v1") }} hr
    on f.user_id_hr = hr.user_id
    and f.headcount_date between hr.employee_start_date and hr.employee_end_date
left join
